...
namespace SharpGlide.Tests.Behaviour.Parts
{
    public class PointPartGroupSupportTests
    {
        ...
        public async Task PointPartStartsChildrenInParallelWay()
        {
            ...
            // Assert
            (sw.Elapsed.TotalSeconds < 3).Should()
                .BeTrue(); // we expect that execution time will be lower than total count of children
        ...
        {
            // Arrange
            var rootPart = new TestPointPartWithGroupSupport { Name = "Root" } as IBasePart;
            var childrenAWithChild = new TestPointPartWithGroupSupport { Name = "Child 1" } as IBasePart;
            var childrenB = new TestPointPartWithGroupSupport { Name = "Child 2" } as IBasePart;
            var childrenC = new TestPointPartWithGroupSupport { Name = "Child 3" } as IBasePart;
            var childrenD = new TestPointPartWithGroupSupport { Name = "Child 4" } as IBasePart;
        ...
        public async Task PointPartCanIterateThroughDirectChildren()
        {
            ...
            rootPart.EnumerateChildren(part => { childrenNames.Add(part.Name); });
        ...
        public async Task PointPartCanIterateThroughAllChildren()
        {
            ...
            // Act
            var childrenNames = new List<string>();
            rootPart.EnumerateChildren(part => { childrenNames.Add(part.Name); }, true);
